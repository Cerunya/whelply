...
export default function BreederFooter({
  kennelName,
  slug,
  themeColor,
  themeAccentColor,
  ...
  const bgColor = themeColor || '#2d5a3d'
  const accentColor = themeAccentColor || '#e0a72e'

  const socials = [
    { href: socialInstagram, icon: <InstagramIcon />, label: 'Instagram' },
    { href: socialFacebook, icon: <FacebookIcon />, label: 'Facebook' },
    { href: socialTiktok, icon: <TiktokIcon />, label: 'TikTok' },
    { href: socialYoutube, icon: <YoutubeIcon />, label: 'YouTube' },
  ].filter((s) => s.href)
