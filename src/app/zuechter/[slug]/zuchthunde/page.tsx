...
export default async function ZuechterZuchthundePage({ params }: { params: { slug: string } }) {
  ...
  const allDogs = await prisma.dog.findMany({
    ...
    include: {
      ...
      media: { orderBy: { sortOrder: 'asc' }, take: 6, select: { url: true, purpose: true } },
    },
    orderBy: { name: 'asc' },
  })
      ...
      <main className="min-h-screen relative">
        ...
        {isOwner && (
          <div className="max-w-5xl mx-auto px-4 py-2 flex justify-end">
            <Link href="/dashboard" className="bg-forest text-white text-xs font-semibold px-4 py-2 rounded-xl hover:bg-forest-light transition-colors">
              Hunde verwalten
            </Link>
          </div>
        )}
              ...
              {studDogs.map((dog) => {
                ...
                return (
                  <Link key={dog.id} href={`/hund/${dog.id}`}
                    className="flex flex-col md:flex-row bg-white rounded-2xl border border-cream-deep overflow-hidden hover:border-blue-300 hover:shadow-md transition-all group h-auto md:h-60">
                    <div className="md:w-64 md:flex-shrink-0 bg-cream-dark overflow-hidden h-48 md:h-full">
                      {img ? (
                    ...
                    <div className="flex-1 p-6 flex flex-col justify-center">
                      ...
                      <h3 className="font-serif text-2xl font-bold text-stone-900 mb-1">{dog.name}</h3>
                      {(dog.color || dog.titles) && <p className="text-sm text-stone-500 mb-2">{[dog.color, dog.titles].filter(Boolean).join(' · ')}</p>}
                      {dog.description && <p className="text-stone-600 text-sm leading-relaxed line-clamp-2">{dog.description}</p>}
                      <p className="text-sm text-forest font-semibold mt-3 group-hover:underline">Profil ansehen {'→'}</p>
                    </div>
                  </Link>
                )
              })}
              ...
              {breedingFemales.map((dog) => {
                ...
                return (
                  <Link key={dog.id} href={`/zuechter/${params.slug}/hund/${dog.id}`}
                    className="flex flex-col md:flex-row bg-white rounded-2xl border border-cream-deep overflow-hidden hover:border-pink-300 hover:shadow-md transition-all group h-auto md:h-60">
                    <div className="md:w-64 md:flex-shrink-0 bg-cream-dark overflow-hidden h-48 md:h-full">
                      {img ? (
                        <img src={img} alt={dog.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                      ) : (
                    ...
                    <div className="flex-1 p-6 flex flex-col justify-center">
                      ...
                      {(dog.color || dog.titles) && <p className="text-sm text-stone-500 mb-2">{[dog.color, dog.titles].filter(Boolean).join(' · ')}</p>}
                      {dog.description && <p className="text-stone-600 text-sm leading-relaxed line-clamp-2">{dog.description}</p>}
                      <p className="text-sm text-pink-500 font-semibold mt-3 group-hover:underline">Mehr erfahren {'→'}</p>
                    </div>
                  </Link>
                )
              })}
