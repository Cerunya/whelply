...
// Gen 3 (Urgroßeltern): Bild + Besitzer (für züchterübergreifende Links)
const ggpInclude = {
  media: { take: 1, select: { url: true } },
  breeder: { select: { subdomain: true, kennelName: true } },
}
// Gen 2 (Großeltern): Bild + Besitzer + deren Eltern
const gpInclude = {
  media: { take: 1, select: { url: true } },
  breeder: { select: { subdomain: true, kennelName: true } },
...
export default async function StammbaumPage({
  ...
  // Slug oder ID akzeptieren (z.B. /welpen/grg-an12/stammbaum)
  ...
  function GrandCard({ dog, role, color }: { dog: (DogLinkTarget & { name: string }) | null; role: string; color: 'pink' | 'blue' }) {
    if (!dog) return (
  ...
  function GreatCard({ dog, role }: { dog: (DogLinkTarget & { name: string }) | null; role: string }) {
    if (!dog) return (
    ...
    <>
      ...
      <main className="min-h-screen bg-cream">
        <div className="max-w-5xl mx-auto px-4 py-10">
          ...
          <div className="mb-6">
            <Link href={`/welpen/${listing.slug || listing.id}`} className="text-sm text-forest font-semibold hover:underline">
              ← Zurück zum Inserat
            </Link>
          </div>
            ...
            <>
              ...
              <>
                ...
                <div className="grid grid-cols-8 gap-2">
                  ...
                  ] as [(DogLinkTarget & { name: string }) | null, string][]).map(([dog, role], i) => (
